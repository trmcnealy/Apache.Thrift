...
namespace Apache.Thrift.Server
{
    ...
    public class TThreadPoolAsyncServer : TServer
    {
        ...
        public struct Configuration
        {
            ...
            public Configuration(int min = DEFAULT_MIN_THREADS,
            ...
            public Configuration(int minWork,
                                 int maxWork,
                                 int minIO,
                                 int maxIO)
            {
        ...
        public TThreadPoolAsyncServer(ITProcessorFactory processorFactory,
            ...
            : this(processorFactory,
                   ...
                   new Configuration(minThreadPoolThreads,
                   ...
                   logger)
        {
            lock(typeof(TThreadPoolAsyncServer))
            {
                if(threadConfig.MaxWorkerThreads > 0 || threadConfig.MaxIOThreads > 0)
                {
                    ...
                    ThreadPool.GetMaxThreads(out work,
                                             out comm);
                    ...
                    if(!ThreadPool.SetMaxThreads(work,
                                                 comm))
                    {
                ...
                if(threadConfig.MinWorkerThreads > 0 || threadConfig.MinIOThreads > 0)
                {
                    ...
                    ThreadPool.GetMinThreads(out work,
                                             out comm);
                    ...
                    if(!ThreadPool.SetMinThreads(work,
                                                 comm))
                    {
            ...
            using(TTransport client = (TTransport)threadContext)
            {
                ...
                try
                {
                    try
                    {
                        ...
                        if(ServerEventHandler != null)
                        {
                            connectionContext = ServerEventHandler.CreateContextAsync(inputProtocol,
                                                                                      outputProtocol,
                        ...
                        while(!stop)
                        {
                            ...
                            if(ServerEventHandler != null)
                            {
                                ServerEventHandler.ProcessContextAsync(connectionContext,
                                                                       inputTransport,
                                                                       cancellationToken).Wait();
                            }

                            //Process client request (blocks until transport is readable)
                            if(!processor.ProcessAsync(inputProtocol,
                                                       outputProtocol,
                                                       cancellationToken).Result)
                            {
                                break;
                    ...
                    if(ServerEventHandler != null)
                    {
                        ServerEventHandler.DeleteContextAsync(connectionContext,
                                                              inputProtocol,
                                                              outputProtocol,
                                                              cancellationToken).Wait();
                    }
