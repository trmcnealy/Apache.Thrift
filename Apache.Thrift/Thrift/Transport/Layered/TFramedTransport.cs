...
namespace Apache.Thrift.Transport
{
    ...
    public class TFramedTransport : TLayeredTransport
    {
        ...
        {
            await InnerTransport.ReadAllAsync(HeaderBuf,
                                              0,
                                              HeaderSize,
                                              cancellationToken);
            ...
            if(0 > size || size > Configuration.MaxFrameSize) // size must be in the range 0 to allowed max
            {
                throw new TTransportException(TTransportException.ExceptionType.Unknown,
                                              $"Maximum frame size exceeded ({size} bytes)");
            }
            ...
            await InnerTransport.ReadAllAsync(bufSegment.Array,
                                              0,
                                              size,
                                              cancellationToken);
        }
        ...
        public override async Task FlushAsync(CancellationToken cancellationToken)
        {
            ...
            BinaryPrimitives.WriteInt32BigEndian(bufSegment.Array,
                                                 dataLen);
